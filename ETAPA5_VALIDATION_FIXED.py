...
print("\n" + "="*80)
print("ETAPA 5 - VALIDAÇÃO FINAL E TESTES COMPLETOS (CORRIGIDO)")
print("="*80)
...
with app.test_request_context():
    for template_name, context in templates_to_test:
        try:
            ...
            if result.count('<') > 5 and 'html' in result.lower():
                compiled_count += 1
                status = "✓ OK"
            else:
                status = "⚠ WARN"
...
route_count = 0
with app.test_request_context():
    for route_name, params in routes_to_check:
        try:
            ...
            error_msg = str(e)[:35]
            print(f"✗ {route_name:<30} → ERRO: {error_msg}")
...
form_checks = {
    ...
    'novo_ativo.html': ['method="POST"', 'action=', 'name="id"', 'name="tipo"'],
    'editar_ativo.html': ['method="POST"', 'name="id"', 'name="tipo"'],
}
    ...
    for template_name, required_elements in form_checks.items():
        try:
            ...
            if found_all:
                ...
                print(f"⚠ {template_name:<30} | Faltando: {missing[0] if missing else 'desconhecido'}")
        except Exception as e:
...
total_checks = compiled_count + route_count + forms_valid + css_ok + files_ok
all_checks = 8 + len(routes_to_check) + len(form_checks) + len(css_checks) + len(critical_files)

if total_checks == all_checks:
    print("\n✓✓✓ VALIDAÇÃO COMPLETA - SISTEMA PRONTO PARA PRODUÇÃO ✓✓✓")
else:
    print(f"\n✓ Validação {total_checks}/{all_checks} checks passaram")
    print("⚠ Revisar qualquer item com ⚠ ou ✗ se necessário")
