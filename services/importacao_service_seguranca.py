...
from services.importacao_service import ServicoImportacao, ResultadoMapeamento
...
class ServicoImportacaoComSeguranca:
    ...
    def gerar_preview_seguro(
        ...
        linhas_dict = []
        for numero_linha, row in linhas:
        ...
        validacao_lote = self.validador_lote.validar_lote(
            linhas=linhas_dict,
            mapeamento_campos={
                m.coluna_original: (m.campo_destino, m.score)
    ...
    def _mapear_linha(
        ...
        """
        Mapeia linha original (chaves originais) para campos de banco.

        Usa resultado_mapeamento.matches para fazer mapeamento.
        """
        linha_mapeada = {}
        ...
        for match in matches:
            ...
            valor_original = row.get(match.coluna_original, '').strip()
